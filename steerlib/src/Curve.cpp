...
Point Curve::useHermiteCurve(const unsigned int nextPoint, const float time)
{
	...
	newPosition = ( 2 * t3 - 3 * t2 + 1 		 ) * controlPoints[prevPoint].position +
			    + (    	t3 - 2 * t2 + normalTime ) * controlPoints[prevPoint].tangent * intervalTime
		      	+ (-2 * t3 + 3 * t2     		 ) * controlPoints[nextPoint].position
		      	+ (     t3 -     t2           	 ) * controlPoints[nextPoint].tangent * intervalTime;
